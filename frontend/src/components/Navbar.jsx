...
import { AnimatePresence, motion } from "motion/react"; // Note: ensure you are using v12+ if using 'motion/react', otherwise use 'framer-motion'
...
const Navbar = () => {
  ...
  const [showCredits, setShowCredits] = useState(false); // FIXED: Capitalized C for consistency
  ...
  const handleSignOut = async () => {
    try {
      // FIXED: Changed 'withCredential' to 'withCredentials' (plural)
  ...
  return (
    <motion.div
      ...
      className={`relative z-20 mx-7 my-2 rounded-2xl bg-gradient-to-br from-black/90 via-black/80 to-black/90 backdrop-blur-2xl border bg-white/10 shadow-[0_22px_55px_rgba(0,0,0,0.75)] flex items-center justify-between px-8 py-4`}
    >
      <div className="flex items-center gap-3">
        <img src="/logo.png" alt="Examnotes" className="w-9 h-9" />
      ...
      <div className="flex items-center gap-6 relative">
        ...
        <div className="relative">
          ...
          <AnimatePresence>
            {showCredits && (
              <motion.div
                key="credits-menu" // FIXED: Added unique key for exit animation
              ...
              >
                ...
                <p className="text-sm text-gray-300 mb-4">
                  Use credits to generate Ai notes, diagrams & pdf.
                </p>
                <button
        ...
        <div className="relative">
          ...
          >
            <span className="text-lg">
              {/* FIXED: Added optional chaining to 'name' to prevent slice on undefined */}
          ...
          <AnimatePresence>
            {showProfile && (
              <motion.div
                key="profile-menu" // FIXED: Added unique key for exit animation
                initial={{ opacity: 0, y: -10, scale: 0.95 }}
                animate={{ opacity: 1, y: 10, scale: 1 }}
                exit={{ opacity: 0, y: -10, scale: 0.95 }}
                transition={{ duration: 0.2 }}
              ...
              >
                ...
                <MenuItem
                  ...
                  onClick={() => { setShowProfile(false); navigate("/questions") }}
                />
                <MenuItem
                  text="🗺️ Career Roadmaps"
                  onClick={() => { setShowProfile(false); navigate("/roadmap") }}
                />
...
// FIXED: Standardized 'onclick' to 'onClick' to match React conventions
function MenuItem({ onClick, text, red }) {
  return (
    <div
      onClick={onClick}
      className={`w-full text-left px-5 py-3 text-sm transition-colors rounded-lg cursor-pointer ${red
